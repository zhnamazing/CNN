...
generate
   for (j = 0; j < NUM_OF_CORE; j = j + 1) 
     begin
    mac_CNN mymac(clk,clk2x,rst,a2d_reg2,data_ad,raddrw_reg2,dram[0+:16],v1[j],macout[j]);  
   end
endgenerate  
    ...
    //mux32
    always @(posedge clk2x or negedge rst)
    begin
      if(!rst)
        data_ds <= 0;
      else begin
...
//    assign neuron = offset_H * colrng + offset_W;//这里neuron是这个值在输出图中的位置，池化后得到新的位置，加进flit中
    ...
    assign tail_pic = (raddr == (core_size+stride*(pool-1))*colrng*core_channel-1) ? 1 : 0;//一个部分算完标志
...
//            count <= count;
        ...
        else if((raddr == (core_size+stride*(pool-1))*colrng * core_channel-1) && tail_edge_reg2 && (!wri))
          begin
            raddr <= 0;
            raddrw <= 0;
//            count <= count;
