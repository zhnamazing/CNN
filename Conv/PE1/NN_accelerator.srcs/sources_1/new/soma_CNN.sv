`timescale 1ns / 1ps
module soma_CNN #(parameter NUM_OF_CORE = 6,core_size = 5,colrng = 32, pool = 2, cut_type = 0)(
    input clk,
    //input clk2x,
    input rst,
    input d2s,
...
bram_bias your_instance_name (
  ...
  .douta(bias)  // output wire [39 : 0] douta
    ...
    //assign v_from_core = (~empty1)&&(~empty2);
...
//  .wr_clk(clk2x),  // input wire wr_clk
...
//);
    ...
    begin
      ...
      for(integer i=0; i< NUM_OF_CORE;i=i+1)
        ...
        begin
            case(cnt_reg2)
              3'd0: pool_win[0] <= (dlut>pool_win[0]) ? dlut : pool_win[0];
              3'd1: pool_win[1] <= (dlut>pool_win[1]) ? dlut : pool_win[1];
              3'd2: pool_win[2] <= (dlut>pool_win[2]) ? dlut : pool_win[2];
              3'd3: pool_win[3] <= (dlut>pool_win[3]) ? dlut : pool_win[3];
              3'd4: pool_win[4] <= (dlut>pool_win[4]) ? dlut : pool_win[4];
              3'd5: pool_win[5] <= (dlut>pool_win[5]) ? dlut : pool_win[5];
              3'd6: pool_win[6] <= (dlut>pool_win[6]) ? dlut : pool_win[6];
              3'd7: pool_win[7] <= (dlut>pool_win[7]) ? dlut : pool_win[7];
            default: pool_win[0] <= pool_win[0];  
        endcase 
      end
      ...
      if(!rst)
        ...
        begin
          if((cnt_loc_reg3 == pool*pool-1)&&d2s_reg3)
            begin
              case(cnt_reg3)
                  3'd0: data_f <= pool_win[0];
                  3'd1: data_f <= pool_win[1];
                  3'd2: data_f <= pool_win[2];
                  3'd3: data_f <= pool_win[3];
                  3'd4: data_f <= pool_win[4];
                  3'd5: data_f <= pool_win[5];
                  3'd6: data_f <= pool_win[6];
                  3'd7: data_f <= pool_win[7];
                 default:data_f <= pool_win[0];
               endcase 
               end
           else
             data_f <= data_f;
