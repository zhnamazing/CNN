`timescale 1ns / 1ps
module dendrite_CNN #(parameter NUM_OF_CORE = 6,core_size = 5,colrng = 32, size = 64, stride = 1,row = 2,group = 4,core_channel=8)(
    input clk,
    //input clk2x,
    input rst,
...
// offset_H <= (((offset_H+1)*stride) > ((group-1)*row - core_size)) ? 0 : (offset_H+1);//先行后列移动窗口
    ...
    localparam [7:0] core = core_size*core_size*core_channel;
    logic [9:0] raddr_reg;
    ...
    logic [7:0]offset_W;
    logic [7:0]raddrw;
    logic [7:0]raddrw_reg;
    logic [7:0]raddrw_reg2;
    logic [7:0]raddrw_reg3;
    logic tail;
    logic flag;
    logic ta;
    logic tail_pic;
    ...
    begin
        ...
        begin
            ...
            case (state1)
                ...
                begin
//                    flag <= flag;
                    if(raddr_reg == (core_size+stride)*colrng*core_channel-1 && !wri)
                      begin
                        d2a<=0; 
                        state1 <= Wait;
                      end
