...
module pe_CNN #(parameter FLIT_WIDTH = 48, CREDIT_WIDTH = 3, ID_BITS = 6,NUM_OF_CORE = 32,core_size = 5,colrng = 14, pool = 2,stride = 1,cut_type = 0,core_channel=8)(
    input clk, 
    //input clk2x,
    input rst,
    input start,
...
//    output logic [30:0] countz
    ...
    localparam [7:0] size = row*colrng*core_channel;
    ...
    axon_CNN  #(NUM_OF_CORE, core_size, colrng, size, stride, row_sobel, group, row, core_channel)inst_axon (
        .clk               (clk),
        //.clk2x             (clk2x),
        .rst               (reset),
        .start             (start),
...
//        .countz            (countz),
    ...
    dendrite_CNN  #(NUM_OF_CORE, core_size, colrng, size, stride, row, group, core_channel)inst_dendrite (
        .clk               (clk),
        //.clk2x             (clk2x),
        .rst               (reset),
        .phase             (phase),
